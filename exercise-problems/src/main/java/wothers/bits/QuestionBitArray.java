package wothers.bits;

// TODO: 数组统一相似问题的研究，"bit位运算" & "求和解法" & "数据相消理论"
// 1. 找数组中唯一出现一次的数字
// 2. 找(连续)数组中唯一缺失的数字
// 3. 找(连续)数组中唯一重复的数字
public class QuestionBitArray {

    // Single Number
    // Non-empty array of integers nums, every element appears twice except for one.
    // Find that single one.
    // Input: nums = [4,1,2,1,2] -> 4
    // 1. 先对数组进行排序, 然后之间判断相邻的两个值 Arrays.sort(nums) O(nlog(n)), O(1)
    // 2. 借助HashSet<>, 通过求和来计算: 2∗(a+b+c)−(a+a+b+b+c)=c   O(n), O(n)
    public static int findSingleNumber(int[] nums) {
        // 正确解法:
        // Bit Manipulation 比特位运算    O(n) O(1)
    ...
    // 理论上，用[0, n]的总值减去nums的总值，差值就是缺失的那个值，sumNums的最大有效值是n，不会溢出
    //
    // 1. Gauss Formula 将上述的计算抽象出来，直接使用length计算总和sum，然后减去数组中值的总和
